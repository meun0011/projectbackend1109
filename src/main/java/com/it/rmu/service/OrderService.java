	...
	public List<OrderResponseModel> getAll() {
	    ...
	    if (orderEntities != null) {
	        for (OrderEntity order : orderEntities) {
	            ...
	            responseObject.setName(order.getName());
	            responseObject.setShippingAddress(order.getShippingAddress());
...
public Integer save(OrderRequestModel request) {
    ...
    if (null != request) {
        ...
        orderEntity.setCreateBy("craem");
        orderEntity.setCreateDate(new Date());
        orderEntity.setUpdateBy("cream S");
        ...
        for (int i = 0; i < productIds.size(); i++) {
            ...
            orderProduct.setCreateBy("cream");
            orderProduct.setCreateDate(new Date());
            orderProduct.setUpdateBy("cream S");
    ...
    if (request != null) {
        ...
        if (optionalOrder.isPresent()) {
            ...
            for (int i = 0; i < productIds.length; i++) {
                ...
                orderProduct.setQuantity(qty);
                orderProduct.setCreateBy("Create By cream");
                orderProduct.setCreateDate(new Date());
                orderProduct.setUpdateBy("update By cream");
                orderProduct.setUpdateDate(new Date());
                orderProductRepository.save(orderProduct);
            }
